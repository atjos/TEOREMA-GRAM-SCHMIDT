...
function renderSteps(steps) {
    ...
    steps.forEach((step, index) => {
        ...
        stepDiv.className = 'step-box p-4 mb-4 rounded-3 border-start border-4 border-info bg-white shadow-sm';
...
function renderFinalSummary(u, e) {
    const formatSet = (arr) => `\\left\\{ ${arr.map(v => "(" + v.map(n => n.toFixed(2)).join(",") + ")").join(", \\\\ ")} \\right\\}`;
    
    document.getElementById('final-ortho').innerHTML = `$$ U = ${formatSet(u)} $$`;
...
function render3DPlot(data) {
    ...
    const layout = {
        ...
        scene: {
            xaxis: { title: 'X', zeroline: true, zerolinecolor: '#000', gridcolor: '#ddd' },
            yaxis: { title: 'Y', zeroline: true, zerolinecolor: '#000', gridcolor: '#ddd' },
            zaxis: { title: 'Z', zeroline: true, zerolinecolor: '#000', gridcolor: '#ddd' },
            aspectmode: 'cube'
        }
    };
